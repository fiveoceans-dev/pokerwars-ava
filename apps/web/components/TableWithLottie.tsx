...
import PlayerSeatWithLottie from "./PlayerSeatWithLottie";
...
export default function TableWithLottie({ timer }: { timer?: number | null }) {
  const {
    playerIds,
    playerHands,
    community,
  ...
  const chipAt = (idx: number) => {
    ...
    const chipPos = getInnerPosition(pos, 0.28);
    const style = {
      left: `${chipPos.x}%`,
      top: `${chipPos.y}%`,
      transform: isMobile 
